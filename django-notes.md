# Django notes
學習 Django 的資源筆記。

## How to install Django

[How to install Django](https://docs.djangoproject.com/en/2.1/topics/install/#installing-official-release)

## Step1

- [Documentation Getting started](https://docs.djangoproject.com/en/2.1/intro/)

- [django入门与实践](https://www.imooc.com/learn/790)

## Step2

- [全面掌握Django ORM](https://www.imooc.com/learn/1087)

- [twtrubiks/django-rest-framework-tutorial](https://github.com/twtrubiks/django-rest-framework-tutorial)

## Step3

-[Customizing authentication in Django](https://docs.djangoproject.com/en/2.1/topics/auth/customizing/)

-[Django之路- 如何開發通用且萬能的的權限框架組件](http://blog.51cto.com/xiagao/1913567)

## 資源

- [mozilla Django 介紹 ](https://developer.mozilla.org/zh-TW/docs/Learn/Server-side/Django/Introduction)

- [twtrubiks/django-tutorial](https://github.com/twtrubiks/django-tutorial)

- [Django筆記(11) - 權限與註冊](http://dokelung-blog.logdown.com/posts/234896-django-notes-11-permission-and-registration)

- [刘江的博客 随笔分类 - Django从入门到精通](https://www.cnblogs.com/feixuelove1009/category/1158337.html)
